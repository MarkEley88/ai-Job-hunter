(() => {
  ...
  function merge(existing,incoming){const result=[...existing];incoming.forEach(job=>{const found=result.find(item=>norm(item.company)===norm(job.company)&&((job.url&&item.url===job.url)||(norm(item.title)===norm(job.title))||(similarity(item.title,job.title)>=.8)));if(!found){result.push(job);return}const sources=[...(found.applyOptions||found.sources||[])];(job.applyOptions||job.sources||(job.url?[{name:job.source||'Source',url:job.url}]:[])).forEach(source=>{if(source?.url&&!sources.some(s=>s.url===source.url))sources.push(source)});found.applyOptions=sources;found.sources=sources;if(!found.url&&job.url)found.url=job.url;if((job.description||'').length>(found.description||'').length)found.description=job.description;if(typeof job.relevanceScore==='number'&&job.relevanceScore>(found.relevanceScore||0)){found.relevanceScore=job.relevanceScore;found.score=job.relevanceScore;found.fitRationale=job.fitRationale}});return result}
  ...
  replacement.addEventListener('click',async()=>{
    ...
    searchStatus.textContent='Searching leadership roles…';
    searchResults.replaceChildren();
    const params=new URLSearchParams({keywords:keywords.join(', '),location,minSalary:'0',_t:String(Date.now())});
    try{
      ...
      save(merge(jobsStore(),enriched));statusFilter.value='all';fitFilter.value='all';statusFilter.dispatchEvent(new Event('change'));
      const sourceText=Array.isArray(data.sources)?data.sources.join(' + '):'available sources';searchStatus.textContent=`${enriched.length} relevant leadership roles found across ${sourceText}.`;
    }catch(error){console.error(error);searchStatus.textContent=`Search API error: ${error.message}.`}finally{replacement.disabled=false}
  });
})();
