async function handler(req, res) {
  ...
  // Search broadly, then rank tightly. Do not use the user's £120k target as an API hard filter:
  // many senior UK roles do not publish salary and would otherwise disappear.
  const terms = [...new Set([
    ...requested,
  ...
  const score = j => {
    ...
    let s = 0;
    if (target.test(j.title || '')) s += 55; else if (related.test(j.title || '')) s += 42;
    if (senior.test(j.title || '')) s += 18;
    if (finance.test(t)) s += 12;
    if (remit.test(t)) s += 10;
  ...
  const relevant = j => {
    ...
    return senior.test(j.title || '') && !excluded.test(j.title || '') && !consulting.test(j.title || '') &&
      (target.test(j.title || '') || related.test(j.title || '') || (remit.test(t) && finance.test(t)));
  };
  ...
  try {
    ...
    const ranked = results.filter(relevant)
      .map(j => ({ ...j, relevanceScore: score(j), fitRationale: 'Senior operations/transformation leadership role; review the remit, location and package for final fit.' }))
